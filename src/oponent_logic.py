...
def oponent_play_round(num_dice_input: int, current_round: round_data) -> round_data:
    rolls = oponent_rolls()
    ...
    for _ in range(rolls):
        ...
        if current_round.next_roll_dice <= 3 or current_round.round_score >= 300:
            log.debug("round ended, high score or few dice left")
            break

        turn.avalible_dice = roll_dice(current_round.next_roll_dice)
